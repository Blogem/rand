...
where
    (
        (`c`.`country_region` = 'Netherlands')
        and (
            (`c`.`province_state` = 'Netherlands')
            or isnull(`c`.`province_state`)
        )
        and (
            (`cl`.`province_state` = 'Netherlands')
            or isnull(`cl`.`province_state`)
        )
        and (
            (`cll`.`province_state` = 'Netherlands')
            or isnull(`cll`.`province_state`)
        )
    )
order by
    `c`.`report_date`